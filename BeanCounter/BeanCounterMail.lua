--[[
	...
	Fork version: 5.9.4961-Revisited.1
...
local function getCustomServerRules()
	local aucAdvanced = _G.AucAdvanced
	local rules = aucAdvanced and aucAdvanced.ServerRules
	if rules and rules.IsEnabled and rules.IsEnabled() then
		return rules
	end
end
...
function private.findStackcompletedAuctions(key, itemID, itemLink, soldDeposit, soldBuy, soldTime)
	...
	soldDeposit, soldBuy, soldTime = tonumber(soldDeposit), tonumber(soldBuy), tonumber(soldTime)
	local oldestPossible = soldTime and soldTime - 208800 --58H 15min oldest we will go back
	...
	local rules = getCustomServerRules()
	if rules then
		local bestMatch
			...
			if i:match(itemString) or i == itemString then
				for index, text in pairs(v) do
					if not text:match(".*USED.*") then
						local postStack, postBid, postBuy, postRunTime, postDeposit, postTime = strsplit(";", text)
						postDeposit, postBuy, postBid, postTime = tonumber(postDeposit), tonumber(postBuy), tonumber(postBid), tonumber(postTime)
						if postDeposit == soldDeposit and postBuy >= soldBuy and postBid <= soldBuy
							and rules.IsEventWithinAuction(postTime, postRunTime, soldTime)
						then
							local expectedExpiration = rules.GetExpectedExpiration(postTime, postRunTime)
							local score = abs(expectedExpiration - soldTime)
							if not bestMatch or score < bestMatch.score then
								bestMatch = {
									entries = v,
									index = index,
									stack = tonumber(postStack),
									bid = postBid,
									score = score,
								}
							end
		...
		if bestMatch then
			tremove(bestMatch.entries, bestMatch.index)
			return bestMatch.stack, bestMatch.bid
		end
...
function private.findStackfailedAuctions(key, itemID, itemLink, returnedStack, expiredTime)
	...
	if rules then
		...
		for i,v in pairs (private.playerData[key][itemID]) do
			if i:match(itemString) or i == itemString then
				for index, text in pairs(v) do
					if not text:match(".*USED.*") then
						local postStack, postBid, postBuy, postRunTime, postDeposit, postTime = strsplit(";", text)
						if returnedStack == tonumber(postStack)
							and rules.MatchesExpiration(postTime, postRunTime, expiredTime)
						then
							local expectedExpiration = rules.GetExpectedExpiration(postTime, postRunTime)
							local score = abs(tonumber(expiredTime) - expectedExpiration)
							if not bestMatch or score < bestMatch.score then
								bestMatch = {
									entries = v,
									index = index,
		...
		if bestMatch then
			tremove(bestMatch.entries, bestMatch.index)
			return bestMatch.stack, bestMatch.bid, bestMatch.buy, bestMatch.deposit
		end
		return
	end
			...
			if i:match(itemString) or i == itemString then
				for index, text in pairs(v) do
					if not text:match(".*USED.*") then
						local postStack, postBid, postBuy, postRunTime, postDeposit, postTime = strsplit(";", text)
						if returnedStack == tonumber(postStack)
							and rules.IsEventWithinAuction(postTime, postRunTime, expiredTime)
						then
							local score = abs(tonumber(expiredTime) - tonumber(postTime))
							if not bestMatch or score < bestMatch.score then
								bestMatch = {
									entries = v,
									index = index,
