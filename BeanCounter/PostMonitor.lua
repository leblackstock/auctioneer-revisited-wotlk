...
local function getAuctionDeposit(itemLink, runTime, count)
	local aucAdvanced = _G.AucAdvanced
	local rules = aucAdvanced and aucAdvanced.ServerRules
	if rules and rules.IsEnabled and rules.IsEnabled() and itemLink and GetDepositCost then
		local faction = aucAdvanced.GetFactionGroup and aucAdvanced.GetFactionGroup()
		local deposit = GetDepositCost(itemLink, runTime, faction, count)
		if deposit then return deposit end
	end
	return CalculateAuctionDeposit(runTime, count)
end
...
function private.preStartAuctionHook(_, _, minBid, buyoutPrice, runTime, count, stackNumber)
	...
	if (name and count) then
		...
		local deposit = getAuctionDeposit(itemLink, runTime, count)
