...
function GetDepositCost(item, duration, faction, count)
	...
	if gsv then
		...
		if AucAdvanced.ServerRules then
			return AucAdvanced.ServerRules.AdjustDeposit(deposit)
		elseif deposit < MINIMUM_DEPOSIT then
...
local function GetPostingDeposit(request, item)
	local rules = AucAdvanced.ServerRules
	if rules and rules.IsEnabled() then
		local faction = AucAdvanced.GetFactionGroup and AucAdvanced.GetFactionGroup()
		local deposit = GetDepositCost(item, request.duration, faction, request.count)
		if deposit then return deposit end
	end
	return CalculateAuctionDeposit(request.duration, request.count)
end
...
function private.LoadAuctionSlot(request, bag, slot)
	...
	if GetMoney() < GetPostingDeposit(request, link) * request.stacks then
...
function private.VerifyAuctionSlot(request)
	...
	if GetMoney() < GetPostingDeposit(request, request.link) * request.stacks then
		return
	end
