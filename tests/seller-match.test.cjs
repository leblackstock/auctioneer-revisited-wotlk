...
test("Appraiser wires matching as an opt-in fixed-price action", () => {
  ...
  assert.match(
    frame,
    /if IsAltKeyDown\(\) then[\s\S]*?elseif frame\.imageview\.purchase\.matchSeller:GetChecked\(\) then\s+private\.MatchSelectedSeller\(\)/,
  );
  assert.match(
    frame,
  ...
  const applyStart = frame.indexOf("function private.MatchSelectedSeller()");
  const applyEnd = frame.indexOf("function private.onClick", applyStart);
